// supabase/functions/base64-decode/index.ts
...
const MAX_INPUT_LENGTH = 700_000;
...
Deno.serve(async (req: Request) => {
  ...
  const endpoint = '/v1/base64/decode';
  ...
  try {
    const binary = atob(body.input);
    const bytes = Uint8Array.from(binary, (c) => c.charCodeAt(0));
    const decoded = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
    await logRequest({ context, requestId: reqId, endpoint, method: 'POST', statusCode: 200, startedAt });
    return ok({ decoded }, reqId);
  } catch {
    await logRequest({ context, requestId: reqId, endpoint, method: 'POST', statusCode: 400, startedAt, errorCode: 'INVALID_INPUT' });
    return fail('INVALID_INPUT', 'input is not valid Base64 or does not decode to valid UTF-8.', reqId);
  }
});
